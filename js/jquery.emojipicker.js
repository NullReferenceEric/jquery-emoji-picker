; (function ($) {
    ...
    $.extend(EmojiPicker.prototype, {
        ...
        listen: function () {
            ...
            // re-set cursor position on user keypress
            var emojiPlugin = this;
            this.$el.keyup(function (e) {
                
                if (e.originalEvent != undefined && $(emojiPlugin.element).is('[contenteditable]')) {
                    var sel = window.getSelection();

                    emojiPlugin.customCaretPosition = sel.anchorOffset;                    
                }
            });
        ...
        updatePosition: function () {
            ...
            */
            ...
            // TB prevent Uploads Studio pickers from appearing off screen
            var uploadStudioDescription = this.settings.customTopPos && this.$el.parents('ytcp-mention-textbox').hasClass('description-textarea');  
            var uploadStudio = this.$container.hasClass('ytcp-uploads-dialog');

            this.$picker.css({
                top: uploadStudioDescription ? this.settings.customTopPos :  diffOffset.top,
                left: uploadStudio ? this.settings.customLeftPos : diffOffset.left
            });
        ...
        iconClicked: function () {
            //tb - added to handle conetnteditable elements
            if ($(this.element).is('[contenteditable]')) {
                var sel = window.getSelection();
                if (this.$picker.is(':hidden')) {                    
                    this.customCaretPosition = sel.anchorOffset;
                } else {
                    sel.collapse(this.$el[0].firstChild, sel.anchorOffset);
                    this.$el.focus();
                }                
            }
        ...
        emojiClicked: function (e) {
            ...
            //tb - added to handle contenteditable elements
            if ($(this.element).is('[contenteditable]')) { 
                if (!this.customCaretPosition) {
                    var sel = window.getSelection();
                    this.customCaretPosition = sel.anchorOffset;
                }
                insertAtCaret(this.element, emojiUnicode, this.customCaretPosition);   
                this.customCaretPosition += emojiUnicode.length;
            } else {
    ...
    function insertAtCaret(inputField, myValue, customCaretPosition) {
        ...
        if (document.selection) {
            ...
            var startPos = customCaretPosition ? customCaretPosition : inputField.selectionStart;
            var endPos = customCaretPosition ? customCaretPosition : inputField.selectionEnd;
        ...
        } else if (customCaretPosition != null) {            

            // tb custom implementation for contenteditable inputs
            var startPos = customCaretPosition;
            var endPos = customCaretPosition;
            var scrollTop = inputField.scrollTop;
            ...
            var sel = window.getSelection();
            sel.collapse(inputField.firstChild, customCaretPosition + myValue.length);
